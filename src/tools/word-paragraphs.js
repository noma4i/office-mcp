...
export const paragraphTools = [
  {
    ...
    inputSchema: {
      type: 'object',
      properties: { limit: { type: 'integer', description: 'Maximum number of paragraphs to list (default: 50)', default: 50 } }
    },
    async handler(args) {
...
`);
  ...
  },
  {
    ...
    inputSchema: { type: 'object', properties: { index: { type: 'integer', description: 'Paragraph index (1-based)' } }, required: ['index'] },
    async handler(args) {
...
`);
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        index: { type: 'integer', description: 'Paragraph index (1-based)' },
        styleName: { type: 'string', description: "Style name (e.g., 'Heading 1', 'Normal')" }
      },
...
`);
  ...
  },
  {
    ...
    annotations: { destructiveHint: true },
    inputSchema: { type: 'object', properties: { index: { type: 'integer', description: 'Paragraph index to delete (1-based)' } }, required: ['index'] },
    async handler(args) {
      const index = validateInteger(args.index, 'index', 1);
      const script = wrapWordScript(`
set d to active document
